...
import GuideReviewDetailModal from "../components/GuideReviewDetailModal";
...
const { data, isLoading, error } = useGuideReviews({ ...filters, page });
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-bold text-primary-700">تقييمات المرشدين</h1>
      ...
      <ReviewFiltersBar
        ...
        placeholder="ابحث باسم المرشد أو السائح..."
      />
          ...
          <table className="w-full text-right border-collapse text-sm">
            <thead>
              <tr className="bg-primary-50 text-primary-600 border-b-2 border-primary-100">
                ...
                <th className="p-3">تاريخ الرحلة</th>
                  ...
                  <tr key={review.id} className="hover:bg-gray-50 border-b border-gray-100">
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        <img
                          src={review.booking?.guide?.avatar || "/logo.png"}
                          className="w-8 h-8 rounded-full object-cover"
                          alt=""
                        />
                        <span className="font-semibold">
                          {review.booking?.guide?.user?.name || "-"}
                        </span>
                      </div>
                    </td>
                    <td className="p-3">{review.booking?.tourist?.name || "-"}</td>
                    <td className="p-3">
                    ...
                    <td className="p-3">{formatDate(review.booking?.start_date)}</td>
