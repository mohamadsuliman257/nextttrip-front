...
export default function TripCard({ trip }: { trip: { trip_places?: TripPlaceItem[]; id: number; title: string; start_date: string | null; end_date: string | null; days: number | null; total_cost: number | null; budget_max: number | null; source: string | null } }) {
                  ...
                  <li key={item.id} className="flex items-start gap-2 rounded-lg bg-white p-2 text-sm">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-slate-400">
                        {item.start_time?.slice(0, 5)} • مدة {item.duration_minutes} دقيقة
